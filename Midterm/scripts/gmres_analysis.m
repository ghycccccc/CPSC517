nx = 20; ny = 20; h = 0.1;
lambda = 1; mu = 1;
fx = rand(nx, ny); fy = rand(nx, ny);

% Build matrix with "Node" ordering
[K_n, b_n] = build_navier_cauchy(nx, ny, h, lambda, mu, fx, fy, 'node');
K_n(1, :) = zeros(1, nx * ny * 2);
K_n(2, :) = zeros(1, nx * ny * 2);
K_n(1, 1) = 1.0;
K_n(2, 2) = 1.0;
b_n(1) = 0;
b_n(2) = 0;

[K_c, b_c] = build_navier_cauchy(nx, ny, h, lambda, mu, fx, fy, 'component');
K_c(1, :) = zeros(1, nx * ny * 2);
K_c(2, :) = zeros(1, nx * ny * 2);
K_c(1, 1) = 1.0;
K_c(2, 2) = 1.0;
b_c(1) = 0;
b_c(2) = 0;


% VERIFY: Is the matrix actually SPD?
% Try a direct solve first. If this fails, the matrix is singular.
try
    x_direct = K_n \ b_n;
    x_direct = K_c \ b_c;
    disp('Direct solver succeeded. Matrix is non-singular.');
catch
    disp('Direct solver failed. Matrix is singular! Check BCs.');
end

% Now run your comparison
compare_representations(K_n, b_n, K_c, b_c);